...
> **Implementation internals:** the ingest path, structured `providerLog` metadata, boot preload, and the `server_logs` / `requests` / `request_attempts` schema are covered in the [Observability deep-dive](../architecture/06-observability.md). This guide covers the operator-facing dashboard, polling API, and client implementation.
